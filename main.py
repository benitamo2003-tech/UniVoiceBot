...
# ================================================================
#  EVENT LOOP دائمی در thread جداگانه
# ================================================================
...
_s = _load_state()
post_reactions    = {int(k): {"likes": set(v["likes"]), "dislikes": set(v["dislikes"])} for k, v in _s["reactions"].items()}
...
async def cb_reaction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "like":
        r["dislikes"].discard(user_id)
        r["likes"].add(user_id)
    else:
        r["likes"].discard(user_id)
        r["dislikes"].add(user_id)
    _save_state()
...
def _call_gemini(prompt, image_bytes=None, file_text=None, voice_bytes=None):
    try:
        if not client:
        ...
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                system_instruction="تو یک دستیار هوش مصنوعی آموزشی هستی. به سوالات درسی، برنامه‌نویسی و علمی به زبان فارسی روان پاسخ بده."
            ),
        )
...
async def cb_anon_enter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text("🕵️ وارد حالت ناشناس شدی.\nهر پیامی بفرستی برای ادمین ارسال می‌شه.", reply_markup=kb)

async def handle_anon_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.message.from_user
    user_id = user.id
    username = f"@{user.username}" if user.username else "بدون یوزرنیم"
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("✉️ پاسخ به این کاربر", callback_data=f"admin_reply_init:{user_id}")]])
    await context.bot.send_message(
        chat_id=ADMIN_ID,
        text=f"🕵️ *پیام ناشناس جدید*\n👤 {user.full_name} | `{user_id}` | {username}\n────────────────\n📝 {update.message.text}",
...
async def cb_admin_reply_init(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _save_state()
    await query.message.reply_text(f"✍️ در حال پاسخ به کاربر `{target_id}` هستید.\nپیام خود را بنویسید:", parse_mode="Markdown")
...
def webhook():
    ...
    future = asyncio.run_coroutine_threadsafe(_ptb_app.process_update(update), _loop)
...
def main():
    ...
    if WEBHOOK_URL:
        future = asyncio.run_coroutine_threadsafe(ptb.initialize(), _loop)
        future.result()
        future2 = asyncio.run_coroutine_threadsafe(
            ptb.bot.set_webhook(url=f"{WEBHOOK_URL}/{TOKEN}"), _loop
        )
        future2.result()
